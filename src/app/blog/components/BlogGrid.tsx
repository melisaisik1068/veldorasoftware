...
const articles = [
  {
    ...
    imageAlt:
      'Code editor with Next.js code, dark background, blue syntax highlighting, professional development environment',
  ...
  },
  {
    ...
    excerpt:
      'ISR, Edge Runtime ve akıllı image optimizasyonu ile Lighthouse 100 hedefi. Gerçek proje vakası.',
    date: '1 Nisan 2026',
    image: 'https://img.rocket.new/generatedImages/rocket_gen_img_1d9d70146-1772246541867.png',
    imageAlt:
      'Performance metrics dashboard on dark screen, green scores, analytics data visualization',
  ...
  },
  {
    ...
    imageAlt:
      'Server infrastructure diagram, dark network topology, blue connection lines, architectural overview',
  ...
  },
  {
    ...
    excerpt:
      'Test, lint, build ve deploy adımlarını otomatikleştiren GitHub Actions workflow tasarımı.',
    date: '14 Mart 2026',
    image: 'https://img.rocket.new/generatedImages/rocket_gen_img_118adf3cd-1775805905832.png',
    imageAlt:
      'CI/CD pipeline visualization on dark background, GitHub workflow diagram, automation flow',
  ...
  },
  {
    ...
    excerpt:
      'REST API yazmadan tam tip güvenli frontend-backend iletişimi. Next.js + tRPC + Zod entegrasyonu.',
    date: '5 Mart 2026',
    image: 'https://img.rocket.new/generatedImages/rocket_gen_img_18cdcabb7-1772464640150.png',
    imageAlt:
      'API code editor showing TypeScript definitions, dark theme, professional developer workspace',
...
export default function BlogGrid() {
  ...
  const filtered =
    activeCategory === 'Tümü' ? articles : articles.filter((a) => a.category === activeCategory);
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                activeCategory === cat
                  ? 'bg-primary text-primary-foreground glow-blue-sm shadow-sm'
                  : 'border border-border bg-background/70 text-muted-foreground hover:text-foreground hover:border-primary/35'
              }`}
            >
              {cat}
            </button>
          ...
          <div className="group rounded-2xl border-glow bg-card overflow-hidden mb-8 cursor-pointer hover:border-accent/30 transition-all duration-500">
            <div className="flex flex-col lg:flex-row">
              ...
              <div className="lg:w-1/2 p-8 md:p-10 flex flex-col justify-center">
                ...
                <h2 className="font-display text-2xl md:text-3xl font-semibold text-foreground leading-tight mb-4 group-hover:text-primary transition-colors">
                  {featured.title}
                </h2>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {featured.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs text-muted-foreground bg-muted px-2.5 py-1 rounded border border-border"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          {rest.map((article) => (
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <h3 className="font-display text-lg font-semibold text-foreground leading-tight mb-3 group-hover:text-primary transition-colors flex-1">
                  {article.title}
                </h3>
