...
const caseStudies = [
{
  ...
  imageAlt: 'Clean minimal e-commerce interface on dark background, blue accent colors, modern UI'
},
{
  tag: 'SaaS Dashboard',
  ...
  imageAlt: 'Analytics dashboard with charts and graphs on dark screen, professional data visualization'
},
{
  ...
  imageAlt: 'Corporate business portal interface, clean professional UI, dark theme with data tables'
}];


export default function PortfolioSection() {
  return (
    <section className="py-20 px-6 bg-[#050A14] relative overflow-hidden">
      {/* Background glow */}
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6 mb-12">
          <div>
            <p className="text-xs text-accent uppercase tracking-widest font-semibold mb-3">Portföy</p>
            <h2 className="font-display text-4xl md:text-5xl font-semibold text-white leading-tight">
              Gerçek Problemler,<br />
              <span className="text-gradient-blue">Gerçek Çözümler</span>
            </h2>
          </div>
          <p className="text-[#64748B] max-w-xs text-sm leading-relaxed">
            Her proje bir teknik meydan okumadır. Çözdüğüm problemi ve ölçülebilir sonuçları paylaşıyorum.
          </p>
        </div>
          ...
          <div
            ...
            style={{ animationDelay: `${i * 0.12}s` }}>
              ...
              <div className="p-6 flex flex-col flex-1">
                <h3 className="font-display text-xl font-semibold text-white mb-4">{cs?.title}</h3>

                {/* Problem → Solution */}
                <div className="space-y-3 mb-5 flex-1">
                  <div className="flex gap-3">
                    <span className="text-xs font-bold text-red-400 uppercase tracking-widest mt-0.5 shrink-0">Problem</span>
                    <p className="text-xs text-[#64748B] leading-relaxed">{cs?.problem}</p>
                  </div>
                  <div className="flex gap-3">
                    <span className="text-xs font-bold text-green-400 uppercase tracking-widest mt-0.5 shrink-0">Çözüm</span>
                    <p className="text-xs text-[#94A3B8] leading-relaxed">{cs?.solution}</p>
                  </div>
                </div>

                {/* Metric */}
                <div className="flex items-center justify-between py-4 border-t border-[#1E3A5F]/40 mb-4">
                  <div>
                    <div className="font-display text-3xl font-semibold text-accent">{cs?.metric}</div>
                  ...
                  <div className="flex flex-wrap gap-1.5 justify-end max-w-[160px]">
                    {cs?.tech?.map((t) =>
                  <span key={t} className="text-[10px] text-[#64748B] bg-[#0F172A] px-2 py-0.5 rounded border border-[#1E3A5F]/30">
                        {t}
                      </span>
                  )}
                  </div>
